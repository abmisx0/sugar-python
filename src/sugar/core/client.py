...
class SugarClient:
    ...
    def export_pools(self, include_block: bool = True) -> Path:
        ...
        return self._export.export_lp_data(df, self._config.name.lower(), block)
    ...
    def export_tokens(self) -> Path:
        ...
        Returns:
            Path to exported file.
        """
        df = self.get_tokens(listed_only=False)
        return self._export.export_tokens(df, self._config.name.lower())

    def export_ve_positions(self, include_block: bool = True) -> Path:
        """
        Export veNFT data to CSV.
        ...
        Raises:
            ContractNotAvailableError: If VeSugar is not available.
        """
        df = self.get_ve_positions()
        block = self.block_number if include_block else None
        return self._export.export_ve_data(df, self._config.name.lower(), block)

    def export_relays(self, include_block: bool = True) -> Path:
        """
        ...
        df = self.get_relays()
        block = self.block_number if include_block else None
        return self._export.export_relay_data(df, self._config.name.lower(), block)

    def export_epochs(self, include_block: bool = True) -> Path:
        """
        Export epoch rewards data to CSV.
