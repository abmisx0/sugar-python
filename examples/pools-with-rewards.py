...
def get_pools_with_rewards(chain: ChainId | str, only_with_rewards: bool = True) -> None:
    ...
    print("This includes pool info, latest epoch rewards, and USD-priced fees/bribes", flush=True)
    ...
    if only_with_rewards:
        print(f"  (Only pools with rewards are included)")
    else:
    ...
    if len(combined_df) > 0 and "votes" in combined_df.columns:
        ...
        for idx, (_, pool) in enumerate(top_pools.iterrows(), 1):
            ...
            reserve0_usd = pool.get("reserve0_usd", 0)
            reserve1_usd = pool.get("reserve1_usd", 0)
            tvl_usd = float(reserve0_usd or 0) + float(reserve1_usd or 0)
            print(
